...
func CancelCommands(ctx uintptr) error {
	...
	if err != nil {
		return errors.New("the specified context handle does not refer to a valid context")
	}
	return nil
}
...
var tpmDeviceNotFound = errors.New("a compatible Trusted Platform Module (TPM) Security Device cannot be found on this computer")
...
var ErrUnavailableLog = errors.New("the TBS event log is not available")

func getCurrentLog(ok bool) ([]byte, error) {
	if !ok || isDrtmBoot() {
		return getBootConfigLog(false)
	}
	return nil, ErrUnavailableLog
}

func getTCGLogs(flag bool) ([]byte, error) {
...
func GetTCGLogEx(logType LogType) ([]byte, error) {
	...
	case LogTypeSRTMBoot:
		return getTCGLogs(false)
	case LogTypeSRTMResume:
		return getTCGLogs(true)
	}
	// ToDo: Implementation getCurrentLogIfResume()
	return nil, ErrUnavailableLog
}
